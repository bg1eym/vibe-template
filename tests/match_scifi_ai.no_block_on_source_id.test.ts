/**
 * match_scifi_ai: source_id 为中文时不阻断，返回 200 + audit warn。
...
describe("match_scifi_ai: source_id 为中文不阻断", () => {
  it("source_id='火星救援' → HTTP 200 + audit.issues 含 source_id 警告", async () => {
    const payload = {
      candidates: [
        makeMatch("火星救援", "火星救援", "Andy Weir"),
    ...
    const sourceIdIssues = (body.data?.audit?.issues ?? []).filter(
      (i) => i.path.includes("source_id") && i.reason.includes("不在候选列表中"),
    );
    expect(sourceIdIssues.length).toBeGreaterThanOrEqual(1);
    expect(sourceIdIssues[0].severity).toBe("warn");
    expect(sourceIdIssues[0].reason).toMatch(/火星救援/);
    expect(sourceIdIssues[0].reason).toMatch(/可用示例/);

    await app.close();
    closeDb(db);
  });

  it("全部 source_id 合法 → audit.issues 无 source_id 警告", async () => {
    const payload = {
      candidates: [
        makeMatch("Dune", "沙丘", "Frank Herbert", "Spice extends life"),
        makeMatch("1984", "一九八四", "George Orwell"),
    ...
    const sourceIdIssues = (body.data?.audit?.issues ?? []).filter((i) =>
      i.path.includes("source_id"),
    );
    expect(sourceIdIssues.length).toBe(0);
