 ...
 * - /atlas: Atlas dashboard — cover image + dashboard URL (triggers atlas:run)
 */
...
const ATLAS_HELP_TEXT = `📡 Atlas Dashboard
...
export default function register(api: OpenClawPluginApi) {
  ...
  api.registerCommand({
    name: "atlas",
    description: "Atlas Dashboard — cover image + dashboard URL",
    acceptsArgs: true,
    requireAuth: true,
    matchNL: (body) => {
      const r = route(body);
    ...
    handler: async (ctx) => {
      ...
      if (r.action === ACTION_ATLAS_TODAY) {
        ...
        if (missing.length > 0) {
          const errText =
            `❌ Atlas 配置缺失。\n\n**缺少:** ${missing.join(", ")}\n\n` +
            "**下一步:** 在 OpenClaw 插件配置或环境变量中设置上述项。";
          api.logger?.info?.(
        ...
        // PCK-ATLAS-005: Cache-first — reply within ~2s if cached result exists
        try {
          ...
          if (latest) {
            ...
            if (coverUrl && !coverMissing) {
              return {
                photoUrl: coverUrl,
                caption: appendFingerprint(`🟦 [打开 Dashboard](${dashboardUrl})\n\n${runIdLine}`),
              } as { photoUrl: string; caption: string };
            }
            return {
              text: appendFingerprint(`🟦 [打开 Dashboard](${dashboardUrl})\n\n${runIdLine}`),
            };
          }
        } catch {
          /* ignore quick cache */
        ...
        api.logger?.info?.(
          `[oc-bind] atlas: action=ATLAS_TODAY source=${source} durationMs=${durationMs} ok=${result.ok} dashboardUrl=${result.dashboardUrl || "—"} coverUrl=${result.coverUrl || "—"} coverMissing=${result.coverMissing}`,
        );
          ...
          if (result.coverMissing) {
            return {
              text: appendFingerprint(
                `🟦 [打开 Dashboard](${result.dashboardUrl})\n\n${runIdLine}\n\n（封面图暂不可用 / cover not available）`,
              ),
            };
          }
          return {
            mediaUrl: result.coverUrl,
            text: appendFingerprint(`🟦 [打开 Dashboard](${result.dashboardUrl})\n\n${runIdLine}`),
          };
        }
        ...
        if (result.error === "timeout") {
          ...
          if (latest && dashboardUrlBase) {
            ...
            return {
              text: appendFingerprint(
                `⏱ Pipeline 超时。使用缓存结果。\n\n🟦 [打开 Dashboard](${dashboardUrl})\n\nrun_id: ${latest}\n\n（pipeline timed out; showing cached dashboard）`,
              ),
            };
          }
        }

        let errText = "❌ Atlas 执行失败。\n\n";
        errText += `**原因:** ${result.error ?? "未知"}\n\n`;
        ...
        errText += `atlas_root=${result.atlasRoot ?? "—"}\n\n`;
        if (result.error === "ATLAS_ROOT_INVALID") {
          errText += "**下一步:** Set ATLAS_ROOT to the atlas-radar repo root (where package.json exists). Run tools/atlas-root-discovery.sh for suggested value.";
        } else {
          errText += "**下一步:** hint: set NODE_BIN or PNPM_JS；或检查 ATLAS_ROOT、ATLAS_DASHBOARD_URL_BASE、ATLAS_COVER_URL_BASE。";
        }
        if (result.stderr) {
